...
class BranchManager(tk.Toplevel):
            ...
            if cached:
                ...
                for item in cached:
                    if len(item) == 3:
                        ...
                        status = ""
                    branches.append(
                        (name, datetime.datetime.fromisoformat(dt), status)
                    )
            else:
                ...
                for pr in pulls:
                    status = (
                        "open"
                        if pr.state == "open"
                        else ("merged" if pr.is_merged() else "closed")
                    )
                    # prefer open status if multiple PRs exist for the branch
                    if pr.head.ref not in status_map or status == "open":
                        status_map[pr.head.ref] = status
                for br in repo.get_branches():
                    dt = br.commit.commit.author.date
                    status = status_map.get(br.name, "")
                    branches.append((br.name, dt, status))
                branch_cache[self.repo_name] = [
                    (b, d.isoformat(), s) for b, d, s in branches
                ]
                save_branch_cache(branch_cache)
    ...
    def apply_filters(self):
        ...
        for name, dt, status in self.branches:
            ...
            self.tree.insert(
                ...
                "end",
                iid=name,
                values=(symbol, name, date_str, status),
            )
