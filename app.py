    ...
    def load_prs(self, state="open", force=False):
        def worker():
            ...
            if not force and cache_key in pr_cache:
                g = Github(token, per_page=100, lazy=True)
                ...
                fetched = list(repo.get_pulls(state=state, sort="created"))
                if state == "closed":
                    fetched = [pr for pr in fetched if pr.raw_data.get("merged_at")]
...
class PullRequestList(tk.Toplevel):
    ...
    def load_prs(self):
        def worker():
            ...
            g = Github(self.token, per_page=100, lazy=True)
            repo = g.get_repo(self.repo_name)
