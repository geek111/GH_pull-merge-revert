...
class BranchManager(tk.Toplevel):
    ...
    def load_branches(self, force=False):
        def worker():
            ...
            if cached:
                branches_info = [(name, datetime.datetime.fromisoformat(dt)) for name, dt in cached]
            else:
                g = Github(self.token, per_page=100)
                repo = g.get_repo(self.repo_name)
                branches_list = list(repo.get_branches())
                branches_info = [(br.name, br.commit.commit.author.date) for br in branches_list]
                branch_cache[self.repo_name] = [(b, d.isoformat()) for b, d in branches_info]
                save_branch_cache(branch_cache)
            ...
            self.after(0, lambda: (self.branches.clear(), self.branch_statuses.clear(), self.apply_filters()))

            for idx, (name, dt) in enumerate(branches_info):
                try:
                    ...
                    status = "error"
                progress = ((idx + 1) / total) * 100 if total else 100
                self.after(0, lambda n=name, d=dt, s=status, p=progress: self.add_branch(n, d, s, p))

            self.after(0, self.finalize_loading)

        self.master.run_async(worker)

    def add_branch(self, name, dt, status, progress):
